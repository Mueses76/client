import React from "react";
import AcademyLogo from "../../../../assets/img/png/academy-logo.png"; // ojo cambiar aqui y la linea 14

import "./PresentationLibros.scss";

/** change name proy */
/**
        change name proy
         */

export default function PresentationLibros() {
  return (
    <div className="presentation-libros">
      <img src={AcademyLogo} alt="Cata-Book" />  
      <p>
        En Cata-Book vas a encontrar los mejores cursos online de
        desarrollo web en Español. Unete a nosotros y empieza tu camino como
        Desarrodor Web o Desarrollador de CMS. Sinceramente, estos curso es el
        tipo de contenido que a mi me hubiera gustado encontrar cuando empecé en
        el mundo del desarrollo web profesional.
      </p>
      <p>¡¡¡Échales un vistazo y aprovecha las ofertas!!!</p>
    </div>
  );
}
